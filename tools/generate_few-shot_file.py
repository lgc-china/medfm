        ...
        for K_shot in K_shot_lst:
            ...
            if dataset_type == 'colon':
                support_set = gen_support_set_twoclass(train_img_infos, K_shot,
                                                       'colon')
                few_shot_lst = []
                # print("1open {}".format(f'colon_{K_shot}-shot_train_exp{exp_num}.txt'))
                with open(f'../configs/data/MedFMC/colon/colon_{K_shot}-shot_train_exp{exp_num}.txt',
                          'w') as f:
                ...
                few_shot_val_lst = []
                # print("2open {}".format(f'colon_{K_shot}-shot_val_exp{exp_num}.txt'))
                with open(f'../configs/data/MedFMC/colon/colon_{K_shot}-shot_val_exp{exp_num}.txt',
                          'w') as f:
                    val_cur = 0
            ...
            print('dataset_type222')
            if dataset_type == 'endo' or dataset_type == 'chest':
                few_shot_lst = []
                # print('3open {}'.format(f'{dataset_type}_{K_shot}-shot_train_exp{exp_num}.txt'))
                with open(
                        f'../configs/data/MedFMC/{dataset_type}/{dataset_type}_{K_shot}-shot_train_exp{exp_num}.txt',
                        'w') as f:
                    for i, i_class in tqdm(enumerate(support_set)):
                        for j_id in support_set[i]:
                            j_pid, j_label = j_id
                            k_line = j_pid
                            few_shot_lst.append(j_id)
                ...
                few_shot_val_lst = []
                # print('4open {}'.format(f'{dataset_type}_{K_shot}-shot_val_exp{exp_num}.txt'))
                with open(f'../configs/data/MedFMC/{dataset_type}/{dataset_type}_{K_shot}-shot_val_exp{exp_num}.txt',
                          'w') as f:
                    val_cur = 0
                    while val_cur < val_num:
                        num = random.randint(0, len(train_img_infos) - 1)
